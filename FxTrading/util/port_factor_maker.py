...
class PortFactorMaker(object):
    def __init__(self, *args, **kwargs):
        self._start_date = kwargs.get('StartDate', date(2001,1,1))
        self._end_date = kwargs.get('EndDate', date.today())
    ...
    def create_feature_vector(self):
        ...
        feature_vec_df = pd.merge(self._create_rsi(),#.diff(), 
                                  self._create_iv_df(delta=25, vi_term='1W'),#.diff(),
                                  right_index=True, left_index=True)
        #feature_vec_df = pd.merge(feature_vec_df, 
        #                          self._create_rsi(), 
        #                          right_index=True, left_index=True)
        ...
        #                          right_index=True, left_index=True)
        #feature_vec_df = pd.merge(feature_vec_df, 
        #                          self._create_carry_df(),
        #                          right_index=True, left_index=True)
        feature_vec_df = pd.merge(feature_vec_df, 
                                  self._create_surprise_df(), 
                                  right_index=True, left_index=True)
        feature_vec_df = pd.merge(feature_vec_df, 
                                  self._create_datachange_df(), 
                                  right_index=True, left_index=True)
        feature_vec_df = pd.merge(feature_vec_df, 
                                  self._create_ctot_df(), 
                                  right_index=True, left_index=True)

        #feature_vec_df = pd.merge(feature_vec_df, 
        #                          self._create_surprise_df().diff(), 
        #                          right_index=True, left_index=True)
        #feature_vec_df = pd.merge(feature_vec_df, 
        #                          self._create_ctot_df().diff(), 
        #                          right_index=True, left_index=True)
        #feature_vec_df = pd.merge(feature_vec_df, 
        #                          self._create_datachange_df().diff(), 
        #                          right_index=True, left_index=True)
        #feature_vec_df = pd.merge(feature_vec_df, 
        #                          self._create_postion_df(),
        #                          right_index=True, left_index=True)
...
if __name__ == '__main__':
    is_weely = True
    #factor_maker = FactorMaker(TargetCcy=['JPY','USD'], is_weekly=is_weely)
    #factor_maker.create_feature_vector().to_csv('feature_vector_usdjpy_{0}.csv'.format('W' if is_weely else 'M'))
    ...
    #import util.common_func as cf
    #return_df['ValueDate'] = cf.convert_date_format(return_df.ValueDate)
    #return_df.set_index('ValueDate', inplace=True)
    #pd.merge(factor_df, return_df, right_index=True, left_index=True).to_csv('factor.csv')
